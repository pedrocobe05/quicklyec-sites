...
export function ServicesPage() {
  ...
  if (loading) {
    return (
      <PageLoadingShell
        eyebrow={copy.services.loading.eyebrow}
        title={copy.services.loading.title}
        description={copy.services.loading.description}
      />
    );
  }
    ...
    <Layout site={data}>
      <main className="mx-auto max-w-6xl px-6 py-12">
        <section className="rounded-[2rem] border border-black/5 bg-[linear-gradient(135deg,rgba(255,255,255,0.96),rgba(245,239,233,0.92))] p-10 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
          <p className="text-xs uppercase tracking-[0.4em] text-[var(--accent)]">{copy.services.eyebrow}</p>
          <h1 className="mt-4 font-serif text-5xl leading-[0.98] text-slate-900 md:text-6xl">{copy.services.title}</h1>
        </section>
          ...
          {data.services.map((service) => (
            <article key={service.id} className="rounded-[2rem] border border-black/5 bg-white/90 p-7 shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[0.68rem] uppercase tracking-[0.35em] text-[var(--accent)]">{copy.services.serviceLabel}</p>
                ...
                <div className="rounded-[1.25rem] bg-[var(--secondary)] px-4 py-3 text-right text-sm text-slate-600 shadow-sm">
                  ...
                  <p className="mt-1 font-medium text-slate-900">{service.price ? `$${service.price}` : copy.services.priceFallback}</p>
                </div>
              </div>
            </article>
          ))}
