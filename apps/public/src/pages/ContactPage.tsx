...
export function ContactPage() {
  ...
  if (loading) {
    return (
      <PageLoadingShell
        eyebrow={copy.contact.loading.eyebrow}
        title={copy.contact.loading.title}
        description={copy.contact.loading.description}
      />
    );
  }
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        <section className="grid gap-8 md:grid-cols-[1.05fr,0.95fr]">
          <div className="rounded-[2rem] border border-black/5 bg-[linear-gradient(135deg,rgba(255,255,255,0.96),rgba(245,239,233,0.92))] p-10 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
            <p className="text-xs uppercase tracking-[0.4em] text-[var(--accent)]">{copy.contact.eyebrow}</p>
            <h1 className="mt-4 font-serif text-5xl leading-[0.98] text-slate-900 md:text-6xl">{copy.contact.title}</h1>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600">{copy.contact.body}</p>
          </div>
          <div className="rounded-[2rem] border border-black/5 bg-white/90 p-8 shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
            <p className="text-[0.7rem] uppercase tracking-[0.35em] text-[var(--accent)]">{copy.contact.channels}</p>
            <div className="mt-6 grid gap-4">
              <div className="rounded-[1.5rem] bg-[var(--secondary)] px-5 py-4">
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500">{copy.contact.email}</p>
                <p className="mt-2 text-lg text-slate-900">{data.tenant.contactEmail ?? 'demo@quicklyecsites.local'}</p>
              </div>
              <div className="rounded-[1.5rem] bg-[var(--secondary)] px-5 py-4">
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500">{copy.contact.phone}</p>
                <p className="mt-2 text-lg text-slate-900">{data.tenant.contactPhone ?? '+593 999 999 999'}</p>
              </div>
              <div className="rounded-[1.5rem] bg-[var(--secondary)] px-5 py-4">
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500">{copy.contact.whatsapp}</p>
